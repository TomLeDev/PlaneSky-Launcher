    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            if(arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLoginLogger.info("Connexion terminée par l'utilisateur")
                return
            }
            ...
            setOverlayContent(
                ...
                "L'authentification à Microsoft s'est mal passé, veuillez réessayer à nouveau.",
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
        ...
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                ...
                setOverlayContent(
                    ...
                    'Pour utiliser le lanceur, vous devez être connecté à au moins un compte. Vous devrez vous connecter à nouveau par la suite.<br><br>Êtes-vous sûr(e) de vouloir vous déconnecter ?',
                    'I\'m Sure',
                    'Cancel'
                )
                setOverlayHandler(() => {
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            if(arguments_.length > 1 && arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLogoutLogger.info("Connexion terminée par l'utilisateur")
                return
            }
            ...
            setOverlayContent(
                "Quelque chose s'est mal passé",
                'La déconnexion a Microsoft a échoué. Veuillez réessayer.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Échec de la suppression<br>Importer un mod ${fullName}`,
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Impossible de basculer<br>Un ou plusieurs mods importés',
                            err.message,
                            'OK'
                        )
                        setOverlayHandler(null)
...
function populateJavaReqDesc(server) {
    settingsJavaReqDesc.innerHTML = `Exige Java ${server.effectiveJavaOptions.suggestedMajor} x64.`
}
...
function populateReleaseNotes(){
    $.ajax({
        ...
        settingsAboutChangelogText.innerHTML = 'Echec de la mise à jour des actualités'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Télécharger<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le lanceur et exécutez le dmg pour mettre à jour.</span>', false, () => {
        ...
        settingsUpdateButtonStatus('Recherchez des mises à jour', false, () => {
            if(!isDev){
